class Component extends DCLogic {
  ...
  move(m) {
    if (m === 'new') return { mvText: 'NIEUW', mvCls: 'new', mvColor: '#F0895B', up: false, down: false };
    if (m > 0) return { mvText: String(m), mvCls: '', mvColor: '#4CBE7E', up: true, down: false };
    if (m < 0) return { mvText: String(-m), mvCls: '', mvColor: '#E9755C', up: false, down: true };
    return { mvText: '–', mvCls: 'flat', mvColor: '#7C7264', up: false, down: false };
  }
  ...
  renderVals() {
    ...
    return {
      ...
      ctx: {
        country: 'Nederland',
        cat: cur.cat,
        catStyle: cur.catOn ? 'color:#A99C8B' : 'color:#7C7264;text-decoration:line-through',
        updated: cur.updated
      },
